...
import { Bell, CalendarDays, Search, MoreHorizontal, SlidersHorizontal, SortAsc, LayoutGrid,  Sun, Moon } from 'lucide-react';
...
export default function Header() {
    ...
    return (
        <div className={`${theme === 'light' ? 'bg-white text-black' : 'bg-[#1F1F22] text-white'}`}>
            {/* Top Bar */}
            <div className="flex flex-col md:flex-row md:items-center md:justify-between py-6 px-4 md:px-6 gap-4">
                <div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1 text-sm text-gray-300 hidden md:flex">
                        <CalendarDays size={18} /> <span>{today}</span>
                    </div>
                    <img src="https://i.pravatar.cc/40?img=3" alt="Profile" className="w-10 h-10 rounded-full border border-gray-700 shadow hidden md:block" />
                </div>
            </div>

            {/* Bottom Controls */}
            <div className="flex justify-between items-center px-4 md:px-6 py-4 border-t border-[#2A2A2E]">
